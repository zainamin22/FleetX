...
public class WordOrderController {
    ...
    @RequestMapping(value = "/workorder_start", method = RequestMethod.GET)
    public ModelAndView showStartWorkOrder(HttpServletRequest request, HttpServletResponse response) {

        ModelAndView mav = new ModelAndView("workorder_start");
        ...
        mav.addObject("order", new GeneralWorkOrders());
    ...
    @RequestMapping(value = "/work_order_process", method = RequestMethod.POST)
    public ModelAndView WordOrderProcess(@ModelAttribute("order") GeneralWorkOrders order, HttpServletRequest request, HttpServletResponse response) throws FileNotFoundException, IOException {
        Integer admin_id = (Integer) request.getSession().getAttribute("admin_id");
      
        issueNotification(admin_id, order);
        WorkOrders i = GeneralWorkOrders.getWorkOrder(order);

        Admin a = adminRepository.findById(admin_id).get();
        i.setAdmin(a);

        try {
            Date date = new SimpleDateFormat("MM/dd/yyyy").parse(order.getDateintext());
            i.setStartTime(date.getTime());
        } catch (ParseException ex) {
            Logger.getLogger(WordOrderController.class.getName()).log(Level.SEVERE, null, ex);
        }

        i.setWstatus(WorkOrderStatus.START_STATUS);

        workorderRepository.save(i);
      ...
      public void issueNotification(int admin_id, GeneralWorkOrders order) throws IOException {
            ...
            public void onDataChange(DataSnapshot ds) {
                ...
                Message message = Message.builder()
                        .setNotification(new Notification(
                                "Work Order Issued ," + order.getWtitle(),
